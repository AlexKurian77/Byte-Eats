...
const GEMINI_API_KEY =
  Constants.expoConfig?.extra?.geminiApiKey || "YOUR_API_KEY";
...
const PricingScreen = () => {
  ...
  const calculatePrice = (item, qty) => {
    const ingredient =
      commonIngredients.find((ing) => ing.name === item) ||
    ...
    try {
      ...
      const parsed = text
        ...
        .map((line) => {
          ...
          const priceMatch =
            parts[1]?.match(
              /₹\s*(\d+(?:\.\d+)?)\s*(?:\/|per)?\s*(kg|g|kgs|grams|l|ml|liter|litre|piece|pc|pcs|pieces|dozen|dz|dzn)?/i
            ) || [];
        ...
        <ScrollView horizontal style={styles.commonIngredientsContainer}>
          {commonIngredients.map((item, index) => (
            <TouchableOpacity
              ...
              style={[
                styles.commonCard,
                selectedItem === item.name && styles.selectedCard,
              ]}
              onPress={() => {

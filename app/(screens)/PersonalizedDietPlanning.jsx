...
const NUTRITIONIX_SEARCH_URL =
  "https://trackapi.nutritionix.com/v2/search/instant";
const NUTRITIONIX_NUTRIENTS_URL =
...
const DietPlanScreen = () => {
  ...
  const getRecipeFromNutritionix = useCallback(async (mealName) => {
    try {
      ...
      console.error(
        `Nutritionix API error for "${mealName}":`,
  ...
  const handleGenerate = async () => {
    if (!profile || !profile.goal || !profile.activityLevel) {
      Alert.alert(
        "Profile Incomplete",
        "Please complete your profile (Goal, Activity Level) before generating a plan."
      );
      return;
    }
    if (!GEMINI_API_KEY) {
      Alert.alert(
        "Configuration Error",
        "Unable to generate plan. API key is missing."
      );
      return;
    }
    ...
    try {
      ...
      if (!result) {
        ...
        throw new Error(
          "The AI couldn't generate a plan this time. Please try again."
        );
      }
      setOutput(result);
      ...
      if (mealNames.length > 0) {
        ...
        const recipePromises = mealNames.map((name) =>
          getRecipeFromNutritionix(name)
        );
    ...
    <View style={styles.profileInfoContainer}>
      <Text style={styles.infoText}>
        ...
        <Text style={styles.infoValue}>
          {getSafeProfileValue(profile?.goal)}
        </Text>
      </Text>
      <Text style={styles.infoText}>
        Activity:{" "}
        <Text style={styles.infoValue}>
          {getSafeProfileValue(profile?.activityLevel)}
        </Text>
      </Text>
      <Text style={styles.infoText}>
        Budget:{" "}
        <Text style={styles.infoValue}>
          {getSafeProfileValue(profile?.budget, "Moderate")}
        </Text>
      </Text>
    </View>
  );
          ...
          <>
            ...
            <TouchableOpacity
              style={[
                ...
                isGeneratingPlan && styles.buttonDisabled,
              ]}
